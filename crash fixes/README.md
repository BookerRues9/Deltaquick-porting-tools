...
{
    // Original Windows Logic (Kept for compatibility)
    if (type == 0)
    {
        timer++;
        var smokeamount = 0;
        if (i_ex(obj_mainchara))
        {
            ...
            var _offset = 400;
            if (room == room_dw_snow_zone) { _room_width = 7600; _offset = 3000; }
    ...
    // ... (rest of type 1 & 2 windows logic)
}
...
if (os_type == os_android)
{
    if (type == 0)
    {
        ...
        if (i_ex(obj_mainchara))
        {
            var rw = room_width;
            var off = 400;
            if (room == room_dw_snow_zone) { rw = 7600; off = 3000; }
            smokeamount = (obj_mainchara.x - off) / rw;
            smokeamount = clamp(smokeamount, 0, 1);
        }
        depth = 5000;
        if (smokeamount > 0.01)
        {
    ...
    if (type == 1 || type == 2)
    {
        ...
        depth = 400000;
        var ball_l = (type == 1) ? 153 : 125;
        var ball_r = (type == 1) ? 182 : 200;
        var ball_y = (type == 1) ? 150 : 1120;
