...
→ Loose base game file (Optimized for eMMC storage. Takes priority over packs).
/files/chapter1_windows/ to /chapter5_windows/
→ Assets required for each specific chapter.
/files/chapterX_windows/game.droid
→ Loose chapter game files (Optimized for eMMC storage. Takes priority over packs).
/files/packs/
→ Contains APK-PACK files with the game.droid for each chapter. The launcher only looks here if the loose game.droid files above are missing.
 /files/mus/
 → Global directory for music files.
/files/deltarune_save_files/ 
→ saved games generated by Deltarune


NOTE: If you want Deltaquick to read the files inside /files/packs/, you must completely delete any loose game.droid files sitting directly in the root or inside the chapterX_windows folders.
...
save fix
To fix the loading save for the each chapter and chapter select :
```gml
